...
$admin_id = $_SESSION['user_id'];  // Assuming the session stores admin's ID for updating their profile
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    // Fetch current admin's data from the users table (assuming both admin and regular users are stored in the same table)
    $sql = "SELECT * FROM users WHERE id = ?";
    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("i", $admin_id); // Use $admin_id to fetch the correct user
        $stmt->execute();
        ...
        if (password_verify($current_password, $user['password'])) {
            ...
            // Update the password in the users table (since admins are stored in the users table)
